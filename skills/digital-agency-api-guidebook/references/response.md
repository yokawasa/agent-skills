...
> **規範強度の注意**:
> - `Content-Type: application/problem+json` の使用は **[SHOULD]**（推奨）。**[MUST]** ではない。
> - `errors` 配列による複数バリデーションエラーの一括返却は**ガイドラインに明記なし**。一般的プラクティスとして紹介する場合は「ガイドラインには規定なし」と注記すること。
...
>
> ⚠️ `errors` 配列でバリデーションエラーを複数返すパターンはガイドラインに明記されていない。実装例として示す場合は「ガイドラインには規定なし・一般的プラクティス」と明示すること。
